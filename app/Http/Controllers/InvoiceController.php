...
class InvoiceController extends Controller
{
    ...
    public function import(Request $request)
    {
        ...
        if(Excel::load($path)->getSheetNames()[0] == 'INVOICE' || Excel::load($path)->getSheetNames()[1] == 'INVOICE'){
            ...
            $flight_check = FlightDetail::selectRaw('i.id as invoice_id, i.charge_type, flight_details.*')
                ->join('flights as f', 'f.id', '=', 'flight_details.flight_id')
                ->join('invoices as i', 'i.from_period', '=', 'f.from_period')
                ->whereRaw('flight_details.id NOT IN (
                SELECT fd.id
            ...
            if($invoice_type == 'Route Charge'){
                for($i = 0 ; $i < count($flight_check) ; $i++){
                    ...
                    if($flight_detail->flight_check_rc == 'billed'){
                        InvoiceDetail::whereRaw('invoice_id = ? and fn_number = ? and departure = ? and arrival = ? and type = ? and date = ?',
                            [$invoice_id, $flight_detail->fn_number, $flight_detail->dep_ap_act, $flight_detail->arr_ap_act, $flight_detail->type, date('Y-m-d', strtotime($flight_detail->arr_act_dt))])
                            ->update([
                            'remarks' => 'SUDAH DITAGIH'
                        ]);
                    }
                    else{
            ...
            elseif($invoice_type == 'TNC'){
                for($i = 0 ; $i < count($flight_check) ; $i++){
                    $flight_detail = FlightDetail::find($flight_check[$i]->id);
                    if($flight_detail->flight_check_tnc == 'billed'){
                        InvoiceDetail::whereRaw('invoice_id = ? and departure = ? and arrival = ? and type = ? and date = ?',
                            [$invoice_id, $flight_detail->fn_number, $flight_detail->dep_ap_act, $flight_detail->arr_ap_act, $flight_detail->type, date('Y-m-d', strtotime($flight_detail->arr_act_dt))])
                            ->update([
                                'remarks' => 'SUDAH DITAGIH'
                            ]);
