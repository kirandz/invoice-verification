...
class TNChargeController extends Controller
{
    ...
    {
        $invoices = Invoice::where('charge_type', 'TNC')->orderBy('from_period')->get();

        return view('tncharge.index', compact('invoices'));
    }
    ...
    public function summary(Request $request)
    {
        if($request->id == "all"){
            $data = InvoiceDetail::selectRaw('invoice_details.service, invoice_details.type, invoices.flight_type, 
                SUM(terminal_charge) as total_charge, COUNT(invoice_details.id) as total_flight')
            ...
            $data = InvoiceDetail::selectRaw('invoice_details.service, invoice_details.type, invoices.flight_type, SUM(terminal_charge) as total_charge, 
    ...
    {
        $data = InvoiceDetail::selectRaw('invoice_details.service, invoice_details.type, invoices.flight_type, SUM(terminal_charge) 
    ...
    public function pivinvoices(Request $request)
    {
        $data = InvoiceDetail::where('invoice_id', $request->id)
            ->whereRaw('(fn_number = ? and departure = ? and arrival = ? and type = ? and fn_updated = ?) or 
                (fn_updated = ? and departure = ? and arrival = ? and type = ?) and invoice_id = ?',
                [$request->fn_number, $request->departure, $request->arrival, $request->type, $request->flight_num,
                    $request->flight_num, $request->departure, $request->arrival, $request->type, $request->id])
            ->orderBy('date')
            ->get();
...
//        Check to Cross
    ...
    public function pivinvupdate(Request $request)
    {
        ...
        $invoice_update = InvoiceDetail::selectRaw('invoice_details.id')
            ->whereRaw('fn_number = (?) and departure = (?) and arrival = (?) and type = (?) and date = (?) and ac_register = (?) and time_out = (?) and charge_type = "TNC"',
                [$invoice->fn_number, $invoice->departure, $invoice->arrival, $invoice->type, $invoice->date, $invoice->ac_register, $invoice->time_out])
            ->join('invoices', 'invoices.id', '=', 'invoice_details.invoice_id')
            ->first();
    ...
    public function downloadcharge($id)
    {
        ...
        $charter = InvoiceDetail::selectRaw('invoice_details.service as Service, invoice_details.type as Type, SUM(terminal_charge) as "Total Charge"')
        ...
        $sum_charter = InvoiceDetail::selectRaw('SUM(x.terminal_charge) as total_charge')
            ->join(DB::raw('(SELECT id, terminal_charge
                FROM invoice_details
        ...
        $hajj = InvoiceDetail::selectRaw('invoice_details.service as Service, invoice_details.type as Type, SUM(terminal_charge) as "Total Charge"')
            ->join(DB::raw('(SELECT id, service, type, date
                    FROM invoice_details
                    WHERE invoice_id = (?)
                    GROUP BY id, service, type, date
        ...
        $sum_hajj = InvoiceDetail::selectRaw('SUM(x.terminal_charge) as total_charge')
            ->join(DB::raw('(SELECT id, terminal_charge
                FROM invoice_details
                WHERE invoice_id = (?) and service = (?))
                as x'),
        ...
        $regular = InvoiceDetail::selectRaw('invoice_details.service as Service, invoice_details.type as Type, SUM(terminal_charge) as "Total Charge"')
            ->join(DB::raw('(SELECT id, service, type, date
                    FROM invoice_details
                    WHERE invoice_id = (?)
                    GROUP BY id, service, type, date
        ...
        $sum_regular = InvoiceDetail::selectRaw('SUM(x.terminal_charge) as total_charge')
            ->join(DB::raw('(SELECT id, terminal_charge
                FROM invoice_details
                WHERE invoice_id = (?) and service = (?))
                as x'),
        ...
        $sum_all = InvoiceDetail::selectRaw('SUM(x.terminal_charge) as total_charge')
            ->join(DB::raw('(SELECT id, terminal_charge
                FROM invoice_details
                WHERE invoice_id = (?))
                as x'),
        ...
        $invoices = InvoiceDetail::selectRaw('aerodrome as Aerodrome, arrival as Arr, airport_from as "From", 
            ...
            time_out as "Time Out", mtow as MTOW, category as Category, tg as TG, total_mtow as "Total MTOW", charge as Charge,
            terminal_charge as "Terminal Charge", remarks as Remarks')
        ...
        $file_name = $month_num.'. '.$month.' ('.$flight_type.') TNC '.$inv_code;
    ...
    {
        return view('tncharge.unbilled');
    }

    public function unbilleddetails(Request $request)
    {
        $check = Invoice::where('charge_type', 'TNC')->get();

        if(count($check) == 0){
            $data = FlightDetail::selectRaw('flight_details.*')
                ->whereRaw('flight_details.arr_sched_dt >= (?) and flight_details.arr_sched_dt <= (?)+INTERVAL 1 DAY 
                    and flight_details.flight_check_tnc = "unbilled" and flight_details.leg_state = "Scheduled"')
                ->setBindings([$request->from_period, $request->to_period])
            ...
            $data = FlightDetail::selectRaw('flight_details.*')
                ->join('flights as f', 'f.id', '=', 'flight_details.flight_id')
                ->join('invoices as i', 'i.from_period', '=', 'f.from_period')
                ->whereRaw('flight_details.arr_sched_dt >= (?) and flight_details.arr_sched_dt <= (?)+INTERVAL 1 DAY and i.charge_type = "TNC" 
                    and flight_details.flight_check_tnc = "unbilled" and flight_details.leg_state = "Scheduled"')
                ->setBindings([$request->from_period, $request->to_period])
                ->groupBy(DB::raw('flight_details.fn_number, flight_details.service, flight_details.ac_registration, flight_details.dep_ap_sched, 
                    flight_details.dep_sched_dt, flight_details.dep_ap_act, flight_details.dep_act_dt, flight_details.arr_ap_sched, 
    ...
    public function downloadunbilled($str)
    {
        ...
        $file_name = $from.' - '.$to.' Unbilled Flight TNC';
